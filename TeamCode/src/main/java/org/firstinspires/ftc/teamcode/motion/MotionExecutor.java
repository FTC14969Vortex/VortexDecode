...
public class MotionExecutor {
    ...
    public void updateState() {
        // Use OdometryManager which applies calibration scaling factors
        odometryManager.update();
        Pose2D calibratedPose = odometryManager.getCurrentPose();
        motionState.updateCurrentPose(calibratedPose);
    }
    ...
    public MotionResult circularMove(double centerX, double centerY, double angularVelocity,
        ...
        // Convert center coordinates based on coordinate mode
        double actualCenterX = centerX;
        double actualCenterY = centerY;
        if (coordinateMode == MotionState.CoordinateMode.ROBOT_CENTRIC) {
            // Convert robot-relative center to field coordinates (reference point frame)
            double headingRad = Math.toRadians(motionState.getHeading());
            double cos = Math.cos(headingRad);
            double sin = Math.sin(headingRad);
            // Transform center from robot frame to field frame
            actualCenterX = startX + (centerX * cos - centerY * sin);
            actualCenterY = startY + (centerX * sin + centerY * cos);
        }
 else {
            // FIELD_CENTRIC: centerX, centerY are already in reference point coordinates
            // Use them directly since all calculations are in reference point frame
        ...
        if (radius < 0.1) {
            ...
            return spinInPlace(totalAngle, angularVelocity);
        }
    ...
    public MotionResult moveToPose(double targetX, double targetY, double targetHeading,
        ...
        while (isExecuting) {
            ...
            // CRITICAL: moveToPose should rotate around REFERENCE POINT, not robot center
            // Keep the reference point at the target position while rotating to target heading
            // This means the robot center will move in a circular arc around the reference point
            FieldPose dynamicRefPointTarget = new FieldPose(targetX, targetY, targetHeading);
            ...
            double currentDistance = motionState.getPositionError();  // Distance from current position to target
            // CRITICAL: Negate distance so PID error = setpoint(0) - (-distance) = +distance, producing positive speed
            double desiredLinearSpeed = driveHardware.getDistanceController().calculate(-currentDistance);
            
            // Calculate direction vector toward target (normalized)
            double deltaX = targetX - motionState.getX();
            double deltaY = targetY - motionState.getY();
            ...
            // Apply velocity command (always in field-centric for moveToPose)
    ...
    public MotionResult combinedMotion(double vx, double vy, double omega,
        ...
        while (isExecuting) {
            ...
            if (maxDistance > 0) {
                double dx = motionState.getX() - startX;
                double dy = motionState.getY() - startY;
    ...
    public MotionResult rotate(double targetAngle) {
        return spinInPlace(targetAngle, MotionConfig.MAX_ANGULAR_VELOCITY);
    }   
   
    public MotionResult rotate(double targetAngle, double maxAngularVelocity) {
        return spinInPlace(targetAngle, maxAngularVelocity);
    }
    
    private MotionResult spinInPlace(double targetAngle, double maxAngularVelocity) {
        // Validate inputs
    ...
    public void resetOdometry() {
        coordinateTransformer.resetToFieldOrigin();
    }
    

    // ========== REFERENCE POINT COORDINATE CONVERSION ==========
    ...
    public void setFieldOrigin(double refPointX, double refPointY, double robotHeading) {
        coordinateTransformer.setFieldOrigin(refPointX, refPointY, robotHeading);
    }
    
    
    public void setFieldOrigin(FieldPose referencePose) {
        coordinateTransformer.setFieldOrigin(referencePose);
    }
