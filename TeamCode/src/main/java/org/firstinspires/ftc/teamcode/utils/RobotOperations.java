...
public class RobotOperations {
    ...
    private static final double MIN_ALIGNMENT_ANGLE = 10.0;        // degrees - only align if > 10 degrees
    ...
    public MotionExecutor.MotionResult moveToShootingPosition(FieldPose targetPos) {
        ...
        try {
            ...
            return baseMotion.moveToPose(targetPos, TRAVEL_VELOCITY);
        } finally {
    ...
    public boolean alignToShootingAngle() {
        ...
        double angleDifference = Math.abs(normalizeAngle(requiredHeading - currentHeading));
        
        // Only align if difference is significant
        if (angleDifference > MIN_ALIGNMENT_ANGLE) {
            if (opMode != null) {
            ...
            MotionExecutor.MotionResult result = baseMotion.rotate(requiredHeading, TRAVEL_VELOCITY);
    ...
    private void applyLocationBehaviors(String locationName, boolean isPreMovement) {
        ...
        // Note: Shooting locations are handled by delegation to moveToShootingPosition()
        // No shooting-specific behaviors needed here
        
        // Future: Add other location-specific behaviors here
        // e.g., gate control for "open_gate", intake activation for "loading", etc.
    }
     ...
     * Stops dynamic flywheel control and robot movement
     */
