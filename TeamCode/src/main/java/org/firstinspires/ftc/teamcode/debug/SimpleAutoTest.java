...
public class SimpleAutoTest extends LinearOpMode {
    ...
    private FieldPose targetFieldPose = FieldPositions.INTAKE_1_START;
    ...
    public void runOpMode() {
        ...
        motionExecutor.resetToFieldOrigin( new Pose2D(
                DistanceUnit.INCH,
                FieldPositions.getReferencePointInitialPosition().x,
                FieldPositions.getReferencePointInitialPosition().y,
                AngleUnit.DEGREES,
                FieldPositions.getReferencePointInitialPosition().heading
        ));
    ...
    private void runTest() {
        ...
        double targetX = targetFieldPose.x;
        double targetY = targetFieldPose.y;
        double targetHeading = targetFieldPose.heading;
        
        // Execute motion
        motionExecutor.moveToPose(targetX, targetY, targetHeading, 30);


       // motionExecutor.moveToPose(targetFieldPose);
    ...
    private void displayResults() {
        ...
        double expectedDeltaX = targetFieldPose.x - initialPose.getX(DistanceUnit.INCH);
        double expectedDeltaY = targetFieldPose.y - initialPose.getY(DistanceUnit.INCH);
        double expectedDeltaHeading = targetFieldPose.heading - initialPose.getHeading(AngleUnit.DEGREES);

        double xError = Math.abs(deltaX - expectedDeltaX);
        double yError = Math.abs(deltaY - expectedDeltaY);
        double headingError = Math.abs(deltaHeading - expectedDeltaHeading);
        ...
        telemetry.addData("  X", "%.2f inches (expectd %.2f inches)",
                finalPose.getX(DistanceUnit.INCH), targetFieldPose.x);
        telemetry.addData("  Y", "%.2f inches (expected %.2f inches)",
                finalPose.getY(DistanceUnit.INCH), targetFieldPose.y);
        telemetry.addData("  Heading", "%.1f degrees (expected %.1f degrees)",
                finalPose.getHeading(AngleUnit.DEGREES), targetFieldPose.heading);
        telemetry.addLine("");
        
        telemetry.addLine("📊 CALCULATED CHANGES:");

        
        telemetry.addData("  ΔX", "%.2f inches (expected: %.2f)", deltaX, expectedDeltaX);
        telemetry.addData("  ΔY", "%.2f inches (expected: %.2f)", deltaY, expectedDeltaY);
        telemetry.addData("  ΔHeading", "%.1f degrees (expected: %.1f)", deltaHeading, expectedDeltaHeading);
        telemetry.addLine("");
